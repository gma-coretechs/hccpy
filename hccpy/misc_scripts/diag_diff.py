...
def write_output(df):
    ...
    logger.info("WRITING: {}".format(output_path + "NW_diag_diff.parquet"))
    df.write.mode('overwrite').parquet(output_path + 'NW_diag_diff.parquet')
    return df
...
def main():
    ...
    cclf1 = cclf1.withColumn("source_year", f.split(f.col("source_file"), "/").getItem(6)).withColumn('year', f.substring(f.col('file_year'), -4, 4))
    cclf4 = cclf4.withColumn("source_year", f.split(f.col("source_file"), "/").getItem(6)).withColumn('year', f.substring(f.col('file_year'), -4, 4))
    cclf5 = cclf5.withColumn("source_year", f.split(f.col("source_file"), "/").getItem(6)).withColumn('year', f.substring(f.col('file_year'), -4, 4))
    cclf8 = cclf8.withColumn("source_year", f.split(f.col("source_file"), "/").getItem(6)).withColumn('year', f.substring(f.col('file_year'), -4, 4))

    cclf1 = cclf1.select('CLM_THRU_DT').withColumn('claim_year', f.substring(f.col('CLM_THRU_DT'), 1, 4))
    cclf4 = cclf4.select('CLM_THRU_DT').withColumn('claim_year', f.substring(f.col('CLM_THRU_DT'), 1, 4))
    cclf5 = cclf5.select('CLM_THRU_DT').withColumn('claim_year', f.substring(f.col('CLM_THRU_DT'), 1, 4))
    cclf8 = cclf8.select('CLM_THRU_DT').withColumn('claim_year', f.substring(f.col('CLM_THRU_DT'), 1, 4))
    ...
    master = master.withColumn('diag_lag',f.lag(df['diagnosis_list']).over(Window.partitionBy("BENE_MBI_ID").orderBy('BENE_MBI_ID','source_year', 'claim_year')))
